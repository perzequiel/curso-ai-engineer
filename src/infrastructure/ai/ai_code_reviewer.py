...
DEFAULT_REVIEW_RULES = [
    ...
    "Los nombres de las variables y funciones deben ser descriptivas",
    ...
    def _build_review_prompt(self, code: CodeContent) -> str:
        ...
        return (
            f"""PR Title: {code.pr_title}
                PR Description: {code.pr_description}
                Folder Structure: {', '.join(code.folder_structure)}
                Files:
                    {files_text}
            """
        )
    ...
    def _parse_response(self, content: str) -> ReviewResult:
        ...
        for i, line in enumerate(lines):
            if line.startswith("RATING:"):
                try:
                    rating = int(line.split(":").strip())
                except (ValueError, IndexError):
                    rating = 50
            elif line.startswith("SUMMARY:"):
                summary = line.split(":", 1)[1].strip()
            elif line.startswith("- ") and any(lines[j].startswith("RECOMMENDATIONS") for j in range(max(0, i - 5), 1)
            ):
